...
export function MapView({ data }) {
  ...
  return (
    <Card style={{ overflow: 'hidden' }}>
      ...
      >
        <span style={{ fontFamily: FONT_D, fontSize: 22, fontWeight: 800, color: C.shu }}>
          {data.kanji}
        </span>
          ...
          {data.lines.map(([a, b, dash], i) => {
            ...
            return (
              <line
                ...
                x2={pb.x}
                y2={pb.y}
                stroke={dash ? '#B9A98A' : C.kin}
                strokeWidth={dash ? 1.4 : 2.4}
                strokeDasharray={dash ? '4 4' : '0'}
                strokeLinecap="round"
                opacity={0.6}
              />
            );
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
            ...
            <span
              style={{
                ...
                fontSize: 10.5,
                fontWeight: 700,
                color: MTYPE[selected.t].c,
                letterSpacing: 0.5,
              }}
            >
              {MTYPE[selected.t].label.toUpperCase()}
            </span>
          </div>
          <div
            style={{
      ...
      >
        {usedTypes.map((t) => (
          <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span style={{ fontFamily: FONT_B, fontSize: 10.5, color: '#777' }}>
              {MTYPE[t].label}
            </span>
          </div>
        ))}
