    ...
    <div>
      ...
      >
        ...
        <div style={{ fontFamily: FONT_B, fontSize: 13, color: '#cdd9e6' }}>
          27 de julio – 8 de agosto · 12 días
        </div>
      </div>
      <SectionLabel kanji="移動" color={C.ai}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 9, marginBottom: 16 }}>
        {TRANSPORT_MOVE.map((t, i) => (
          <TransportRow key={i} t={t} />
        ))}
      </div>

      <SectionLabel kanji="日帰" color={C.matcha}>
        Excursión de un día
      </SectionLabel>
      <p
        style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 9, marginBottom: 12 }}>
        {TRANSPORT_DAYTRIP.map((t, i) => (
          <TransportRow key={i} t={t} />
        ))}
      </div>

      <Callout title="🎟️ ¿Cuáles reservar con anticipación?" color={C.shu} bg={TAGS.SALIR.bg}>
        Solo los 3 marcados <strong>"RESERVAR ANTES"</strong>: el Romancecar a Hakone, el Shinkansen
        a Kioto y el Haruka al aeropuerto. Son trenes con asiento asignado; resérvalos por la app
        SmartEX o en la estación 1-2 días antes (en verano se llenan).
      </Callout>
      <Callout title="🎫 El resto: solo tarjeta IC" color={C.matcha} bg={TAGS.ZEN.bg}>
        Los marcados <strong>"sin reserva"</strong> (Kioto↔Osaka, Kioto↔Nara, Haneda→Tokio) son
        trenes normales: pasas la tarjeta IC (Welcome Suica o ICOCA) y te subes. Sin trámite.
      </Callout>
      {JP_CITIES.map((c, i) => (
        <div key={i} style={{ marginTop: 26 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
            <Seal color={c.color} size={50} rot={-5}>
              <span style={{ fontSize: 22 }}>{c.kanji}</span>
            </Seal>
            <div>
              <h2
                style={{
          ...
          {c.map && (
            <div style={{ marginBottom: 14 }}>
              <MapView data={MAPS[c.map]} />
            </div>
          )}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 12 }}>
            {c.days.map((d, j) => (
              <DayCard key={j} d={d} accent={c.color} />
            ))}
          </div>
        </div>
      ))}
